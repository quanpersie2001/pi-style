...
| `messages.hideThinkingLabel` | boolean | `true` | Hide Pi's `Thinking...` placeholder label for hidden thinking blocks with zero trace: the certified `updateContent` patch (recorded for Pi `0.83.0` through `0.85.0`) drops the invisible label row and its trailing spacer, leaving the same single top padding as a text-only assistant message. Set `false` to restore the default label. |
